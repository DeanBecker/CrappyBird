...
VertexArray* Pipe::model;
Texture* Pipe::texture;
Shader* Pipe::pipeShader;
...
void Pipe::update()
{
	...
	mlMatrix = mlMatrix->multiply(translateMat);
	delete translateMat;
}
...
void Pipe::render()
{
	...
	pipeShader->setUniformMat4f("ml_mat", mlMatrix);
